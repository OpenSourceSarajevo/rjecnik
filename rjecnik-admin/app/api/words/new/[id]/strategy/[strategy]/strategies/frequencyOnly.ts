...
export async function applyFrequencyOnly(id: number, body: any) {
  ...
  const headword: Word = headwordData;

  const { data: headwordUpdated, error: headwordUpdatedError } = await supabase
    .from("words_v2")
    .update({ 
      frequency: headword.frequency + newWord.count, 
      updated_by: body.user_email,
      updated_at: new Date().toISOString()
    })
    .eq("headword", headword.headword)
    .select();
